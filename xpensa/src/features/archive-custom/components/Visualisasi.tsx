...
import { useCustomBarChart } from '../hooks/barChart.hooks';
import { useCustomPieChart } from '../hooks/pieChart.hooks';
...
export const Visualisasi = ({
  ...
  const { loading, chartData, totalExpense } = useCustomBarChart(
    userId,
    startDate,
    endDate,
  );

  const { loadingPie, pieData, totalPieCount } = useCustomPieChart(
    userId,
    startDate,
    endDate,
  );
      ...
      <View className='flex-1 gap-y-[50px]'>
        <View className='bg-white'>
          <View className='w-[90%] mx-auto mt-[15px]'>
            <View className='flex gap-y-[5px] mb-[30px]'>
